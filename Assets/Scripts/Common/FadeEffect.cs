...
{
    /// <summary>
    /// SpriteRenderer의 알파값을 일정 시간 동안 보간하여 변경하는 메소드.
    /// </summary>
    /// <param name="target">대상 2D 오브젝트</param>
    /// <param name="start">시작 투명도</param>
    /// <param name="end">목표 투명도</param>
    /// <param name="fadeTime">지속 시간</param>
    public static async UniTask Fade(SpriteRenderer target, float start, float end, float fadeTime = 1f)
    {
        if (target == null)
        {
            return;
